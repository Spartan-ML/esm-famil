          ...
          >
            ...
            >
              <input
                ...
                placeholder="000000"
                autoFocus
                className={`w-full px-4 py-5 rounded-2xl border text-3xl font-black text-center tracking-[0.4em] outline-none transition-all
                  bg-white/5 ${theme.border} ${theme.text} placeholder:text-current placeholder:opacity-20
                ...
                {Array.from({ length: 6 }).map((_, i) => (
                  <motion.div
                    ...
                    className={`w-2.5 h-2.5 rounded-full transition-all ${
                      i < code.length ? theme.bgStrong : theme.bgMuted
                    }`}
            ...
            >
              {loading
                ? <span className="flex items-center justify-center gap-2"><Spinner />{t.joining}</span>
                : `${t.join} →`
              }
            </motion.button>
          </motion.div>
        ...
        {step === "profile" && (
          ...
          >
            ...
            {selectedColor && (
              ...
              >
                <div
                  className="w-5 h-5 rounded-full shadow border-2 border-white/30"
            ...
            <motion.input
              ...
              className={`w-full px-4 py-4 rounded-2xl border text-lg font-semibold outline-none transition-all
                bg-white/5 ${theme.border} ${theme.text} placeholder:text-current placeholder:opacity-30
                focus:ring-2 ${theme.ring} focus:border-transparent`}
            />
            ...
            <motion.button
              whileHover={name.trim() && selectedColor ? { scale: 1.02 } : {}}
              whileTap={name.trim() && selectedColor ? { scale: 0.97 } : {}}
              onClick={handleJoin}
              disabled={!name.trim() || !selectedColor || loading}
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.35 }}
              className={`w-full py-4 rounded-2xl text-base font-bold transition-all
                ${name.trim() && selectedColor && !loading
                  ? `${theme.button} ${theme.buttonHover}`
                  : `${theme.bgMuted} ${theme.textMuted} opacity-50 cursor-not-allowed`
                }`}
            >
